...
const songTypeOptions = document.getElementById('songTypeOptions'); // this will hold theme filters
const searchInput = document.getElementById('search');

let activeTypes = [];
let activeSongTypes = []; // themes
let activeAreas = [];
let searchQuery = '';
let currentAudio = null;
let currentButton = null;

// Type colors (kept from your code)
const typeColors = {
  Normal: "white",
  Plant: "#6BBF59",
  Water: "#3BA5FF",
...
// convert "Lake - MemeTheme" -> { area: "Lake", theme: "MemeTheme" }
// If right-side contains "Acapella" -> "AcapellaTheme", contains "Meme" -> "MemeTheme"
// Otherwise keep the right side (Theme, BattleTheme, etc.)
function splitTypeField(t) {
  ...
  if (right.length > 0) return { area, theme: right.replace(/\s+/g, '') }; // e.g. "BattleTheme", "GauntTheme"
...
fetch('../sData/songs.json')
  ...
  .then(data => {
    ...
    songs = data.map(s => {
      ...
      // normalize typing -> array
      if (!Array.isArray(song.typing)) {
        song.typing = song.typing ? [song.typing] : ['Unknown'];
      }

      // many earlier files used 'type' field; normalize to both area & theme
      ...
      // legacy 'types' -> remove or ignore (we now use .theme and .area)
      return song;
    });
...
// Render songs (filter + sort by order)
function renderSongs() {
  ...
  // Sorting logic based on toggle
if (sortMode === "OST") {
  filtered.sort((a, b) => (Number(a.ost) || 0) - (Number(b.ost) || 0));
} else {
  ...
  filtered.forEach(song => {
    ...
    // Title / composer / area / theme
    const title = document.createElement('h3');
    title.textContent = song.name;
    const composer = document.createElement('p');
    composer.className = 'composer';
    composer.textContent = `Composer: ${song.composer || 'Unknown'}`;

    const areaEl = document.createElement('div');
    areaEl.className = 'types'; // reuse styles
    const areaSpan = document.createElement('span');
    areaSpan.textContent = song.area || 'Unknown';
    areaEl.appendChild(areaSpan);

    const themeEl = document.createElement('div');
    themeEl.className = 'types';
    const themeSpan = document.createElement('span');
    themeSpan.textContent = song.theme || 'Theme';
    themeEl.appendChild(themeSpan);
    ...
    handle.classList.add('progress-handle');
    progressContainer.appendChild(handle);
    ...
    // Pointer-based dragging (works better than mouse events and supports touch)
    let dragging = false;
    ...
    handle.addEventListener('pointermove', (ev) => {
      ...
      // set currentTime live while dragging (so update reflects)
    ...
    handle.addEventListener('pointerup', (ev) => {
      ...
      // ensure final time set
      const rect = progressContainer.getBoundingClientRect();
      let newPercent = ((ev.clientX - rect.left) / rect.width);
      newPercent = Math.max(0, Math.min(1, newPercent));
      if (audio.duration) audio.currentTime = newPercent * audio.duration;

      // Pause other audios and mark this one active (so only one plays)
      if (currentAudio && currentAudio !== audio) {
        currentAudio.pause();
        if (currentButton) currentButton.textContent = 'Play';
      }
    ...
    // Click-to-seek on the whole container
    progressContainer.addEventListener('click', (e) => {
      ...
      // if clicking to seek should also start playing, do the single-play logic:
      if (currentAudio && currentAudio !== audio) {
        currentAudio.pause();
        if (currentButton) currentButton.textContent = 'Play';
      }
...
document.getElementById("sortToggle").addEventListener("click", () => {
  ...
  // Re-render the song list with new sorting method
  renderSongs();
});
...
// Filters initialization (typing, theme, area)
function initFilters() {
  ...
  // theme checkboxes (songTypeOptions)
  themes.forEach(th => {
    const label = document.createElement('label');
    label.className = 'filter-label';
  ...
  // create area panel inside the DOM if not present
  let areaOptions = document.getElementById('areaOptions');
  if (!areaOptions) {
    // fallback: find controls container and append one similar to songTypeWrapper
    const controls = document.querySelector('header .controls');
    if (controls) {
      const wrapper = document.createElement('div');
      wrapper.className = 'multi-filter';
      wrapper.innerHTML = `
        <button class="filter-toggle" id="areaToggle">Area ▾</button>
        <div class="filter-panel" id="areaPanel">
          <div class="panel-actions"><button id="clearAreas" class="clear-btn">Clear</button></div>
  ...
  if (areaOptions) {
    ...
    if (clearAreasBtn) {
      clearAreasBtn.addEventListener('click', () => {
        activeAreas = [];
        areaOptions.querySelectorAll('input').forEach(i => i.checked = false);
        renderSongs();
      });
    }
  }

  // existing clear buttons
  const clearTypesBtn = document.getElementById('clearTypes');
  if (clearTypesBtn) {
    clearTypesBtn.addEventListener('click', () => {
      activeTypes = [];
      typeOptions.querySelectorAll('input').forEach(i => i.checked = false);
      renderSongs();
    });
  }
  const clearSongTypesBtn = document.getElementById('clearSongTypes');
  if (clearSongTypesBtn) {
    clearSongTypesBtn.addEventListener('click', () => {
      activeSongTypes = [];
      songTypeOptions.querySelectorAll('input').forEach(i => i.checked = false);
      renderSongs();
    });
  }
}
